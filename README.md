...
Im Raumkonfigurator lassen sich Geräte der Home-Assistant-Integration [WashData](https://github.com/3dg1luk43/ha_washdata) unter **WashData-Geräte** auswählen. Ein WashData-Gerät ohne Home-Assistant-Bereich kann genau einem CouchMate-Raum zugeordnet werden, ohne seine Home-Assistant-Zuordnung zu ändern. Das Auswahlmodell speichert die Geräte-IDs unter `selection_model.areas.<area_id>.washdata_devices`. Der Core überträgt die ausgewählten Anzeige-Sensoren und liefert im gekoppelten Client-Payload `washdata_appliances` mit `device_id`, `name`, `area_id`, `area_name` und `sensor_entity_ids`. Letzteres ordnet die Rollen `state`, `program`, `time_remaining`, `cycle_progress`, `current_phase` und `total_duration` den aktuellen Home-Assistant-Entitäts-IDs zu. Die Zuordnung basiert auf WashDatas Gerätekennung und den stabilen Entity-Registry-Unique-IDs; umbenannte Entitäten bleiben damit erkennbar. Die Restzeit ist eine von WashData geschätzte Minutenangabe und kein Home-Assistant-`timer.*`.
