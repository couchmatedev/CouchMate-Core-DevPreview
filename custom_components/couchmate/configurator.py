...
class CouchMateConfiguratorSaveView(HomeAssistantView):
    ...
    async def post(self, request):
        ...
        for area_id, raw_area in raw_areas.items():
            ...
            if room_thermostat_card_style in ("full_vertical", "full", "compact", "hidden"):
            ...
            if isinstance(raw_hero_layout, dict):
                allowed_layout_styles = {
                    "thermostat_card_style": {
                        "full_vertical", "full", "compact", "hidden"
                    },
...
function renderThermostatStyles(panel,grid,current,setValue,group,allowInherit){grid.innerHTML='';panel.classList.remove('hidden');const choices=[];if(allowInherit)choices.push({id:null,name:'Globalen Standard verwenden',subtitle:'Übernimmt die Auswahl auf der Raumübersicht.'});choices.push({id:'full_vertical',name:'Vollständig · vertikal',subtitle:'Istwert, Sollwert, Luftfeuchte, Heizstatus und Modi auf 340 pt Breite.'},{id:'full',name:'Vollständig',subtitle:'Istwert, Sollwert, Luftfeuchte, Heizstatus und verfügbare Modi.'},{id:'compact',name:'Kompakt',subtitle:'Schmale Temperatursteuerung mit weniger Zusatzinformationen.'},{id:'hidden',name:'Ausblenden',subtitle:'Kein Thermostat im Hero; die Raumsteuerung bleibt verfügbar.'});for(const choice of choices){const row=document.createElement('label');row.className='choice';row.innerHTML=`<input type="radio" name="${group}"><span><b>${esc(choice.name)}</b><span class="sub">${esc(choice.subtitle)}</span></span>`;const input=row.querySelector('input');input.checked=(current??null)===(choice.id??null);input.onchange=()=>setValue(choice.id);grid.append(row)}}
