...
class CouchMateConfiguratorSaveView(HomeAssistantView):
    ...
    async def post(self, request):
        ...
        for area_id, raw_area in raw_areas.items():
            ...
            if area_cfg.get("temperature") or area_cfg.get("humidity") or area_cfg.get("climate") or area_cfg.get("thermostat_card_style") or area_cfg.get("hero_order") or area_cfg.get("flow_entities") or area_cfg["devices"]:
...
<div id="flowPanel" class="config-panel"><h3>Flow-Fallback</h3><p class="sub">Automatische Aktionen haben Vorrang. Wähle bis zu drei Funktionen, die freie Plätze im Flow dieses Raums füllen.</p><div id="flowGrid" class="entities"></div></div>
...
const selected={devices:new Map(),temperatures:{},humidities:{},climates:{},orders:{},flows:{},layouts:{},thermostatStyle:'full',thermostatStyles:{},showRoomName:true,showRoomClimate:true,weather:null};
...
function renderFlowChoices(){flowGrid.innerHTML='';const candidates=area.flow_candidates||[];flowPanel.classList.toggle('hidden',!candidates.length);const ids=selected.flows[area.id]||[];for(const e of candidates){const row=document.createElement('label');row.className='entity';row.innerHTML=`<input type="checkbox"><div class="icon" style="width:34px;height:34px;margin:0">${icon(e.domain)}</div><span><b>${esc(e.name)}</b><span class="sub">${esc(e.entity_id)}</span></span>`;const c=row.querySelector('input');c.checked=ids.includes(e.entity_id);c.disabled=!c.checked&&ids.length>=3;c.onchange=()=>{const next=[...(selected.flows[area.id]||[])];const index=next.indexOf(e.entity_id);if(c.checked&&index<0&&next.length<3)next.push(e.entity_id);if(!c.checked&&index>=0)next.splice(index,1);selected.flows[area.id]=next;renderFlowChoices()};flowGrid.append(row)}}
